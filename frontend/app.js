...
function init() {
    ...
    const savedSession = localStorage.getItem('sugar_session');
    ...
    if (savedSession) {
        ...
        currentState.currentUser = session;
    }
...
function handleLogin() {
    currentState.isLoggedIn = true;
    currentState.currentUser = { name: '홍길동', role: '임대인' };
    localStorage.setItem('sugar_session', JSON.stringify(currentState.currentUser));
    updateAuthUI();
    updateEditControlsVisibility();
    renderMenu(); // Re-render to show drag handlers
}
...
function updateAuthUI() {
    if (currentState.isLoggedIn) {
        authSection.innerHTML = `
            <div class="user-account" style="display: flex; align-items: center; gap: 0.75rem; background: #eef2ff; padding: 0.4rem 0.8rem; border-radius: 20px; border: 1px solid #c7d2fe;">
                <div class="user-avatar" style="width: 24px; height: 24px; background: #4f46e5; color: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 0.7rem; font-weight: 600;">H</div>
                <span style="font-size: 0.85rem; font-weight: 500;">${currentState.currentUser.name} 님</span>
                <button id="logout-btn" class="btn-ghost" style="padding: 0; font-size: 0.8rem; cursor: pointer; border: none; background: transparent;">로그아웃</button>
            </div>
        `;
        document.getElementById('logout-btn').addEventListener('click', handleLogout);
    } else {
        authSection.innerHTML = `<button id="login-btn" class="btn btn-outline">로그인</button>`;
...
function handleLogout() {
    ...
    localStorage.removeItem('sugar_session');
    updateAuthUI();
    updateEditControlsVisibility();
    exitEditMode();
    renderMenu(); // Re-render to hide drag handlers
}
